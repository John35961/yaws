...
def call_apis(location_lat, location_lon):
    ...
    opwm_cel_api = get(
        ...
        f"&appid={application.config['OPWM_API_KEY']}&units=metric"
    ).json()

    opwm_fah_api = get(
        f"https://api.openweathermap.org/data/2.5/weather"
        f"?lat={location_lat}&lon={location_lon}"
        f"&appid={application.config['OPWM_API_KEY']}&units=imperial"
    ).json()

    opwm_cel_forecast_api = get(
        f"https://api.openweathermap.org/data/2.5/forecast"
        f"?lat={location_lat}&lon={location_lon}"
        f"&appid={application.config['OPWM_API_KEY']}&units=metric"
    ).json()

    opwm_fah_forecast_api = get(
        f"https://api.openweathermap.org/data/2.5/forecast"
        f"?lat={location_lat}&lon={location_lon}"
        f"&appid={application.config['OPWM_API_KEY']}&units=imperial"
    ).json()

    opwm_uv_index_api = get(
    ...
    timezonedb_api = get(
        ...
        f"&key={application.config['TIMEZONEDB_API_KEY']}"
    ).json()

    air_quality_api = get(
        f"http://api.airvisual.com/v2/nearest_city"
        f"?lat={location_lat}&lon={location_lon}"
        f"&key={application.config['AIRQUALITY_API_KEY']}"
    ).json()
...
def store_data_from(
    ...
    apis_data["location_local_time"] = datetime.strptime(
        timezonedb_api["formatted"], "%Y-%m-%d %H:%M:%S"
    )
    apis_data["weather_time_calc_utc_current"] = datetime.fromtimestamp(
        opwm_cel_api["dt"]
    ).strftime("%d/%m/%Y, at %H:%M")
    apis_data["weather_time_calc_utc_forecast"] = [
        datetime.strptime(time_calc["dt_txt"], "%Y-%m-%d %H:%M:%S")
        for time_calc in opwm_cel_forecast_api["list"]
    ]
    apis_data["weather_description"] = opwm_cel_api["weather"][0][
        "description"
    ].capitalize()
    apis_data["weather_pressure"] = opwm_cel_api["main"]["pressure"]
    apis_data["weather_humidity"] = opwm_cel_api["main"]["humidity"]
    apis_data["weather_air_quality_index"] = air_quality_api["data"]["current"][
        "pollution"
    ]["aqius"]
    apis_data["country_emoji_flag"] = flag.flagize(
        f":{opwm_cel_api['sys']['country']}:"
    )
    ...
    apis_data["weather_cel_temp_forecast"] = [
        round(temp["main"]["temp"], 1) for temp in opwm_cel_forecast_api["list"]
    ]
    apis_data["weather_cel_temp_forecast_mean"] = round(
        mean(apis_data["weather_cel_temp_forecast"]), 1
    )
    apis_data["weather_cel_temp_forecast_min"] = round(
        min(apis_data["weather_cel_temp_forecast"]), 1
    )
    apis_data["weather_cel_temp_forecast_max"] = round(
        max(apis_data["weather_cel_temp_forecast"]), 1
    )
    apis_data["weather_cel_feels_like"] = round(opwm_cel_api["main"]["feels_like"], 1)
    apis_data["weather_cel_feels_like_forecast"] = [
        round(temp["main"]["feels_like"], 1) for temp in opwm_cel_forecast_api["list"]
    ]
    ...
    apis_data["weather_fah_temp_forecast"] = [
        round(temp["main"]["temp"], 1) for temp in opwm_fah_forecast_api["list"]
    ]
    apis_data["weather_fah_temp_forecast_mean"] = round(
        mean(apis_data["weather_fah_temp_forecast"]), 1
    )
    apis_data["weather_fah_temp_forecast_min"] = round(
        min(apis_data["weather_fah_temp_forecast"]), 1
    )
    apis_data["weather_fah_temp_forecast_max"] = round(
        max(apis_data["weather_fah_temp_forecast"]), 1
    )
    apis_data["weather_fah_feels_like"] = round(opwm_fah_api["main"]["feels_like"], 1)
    apis_data["weather_fah_feels_like_forecast"] = [
        round(temp["main"]["feels_like"], 1) for temp in opwm_fah_forecast_api["list"]
    ]
    ...
    apis_data["table_forecast"] = {
        "table_weather_description": [
            descr["weather"][0]["description"].capitalize()
            for descr in opwm_cel_forecast_api["list"][::3]
        ],
        "table_weather_icon": [
            ico["weather"][0]["icon"] for ico in opwm_cel_forecast_api["list"][::3]
        ],
        "table_weather_humidity": [
            level["main"]["humidity"] for level in opwm_cel_forecast_api["list"][::3]
        ],
        "table_weather_cel_wind_speed": [
            round(speed["wind"]["speed"], 1)
            for speed in opwm_cel_forecast_api["list"][::3]
        ],
        "table_weather_fah_wind_speed": [
            round(speed["wind"]["speed"], 1)
            for speed in opwm_fah_forecast_api["list"][::3]
        ],
    }
    ...
    try:
        apis_data["weather_wind_direction_abbr"] = portolan.point(
            degree=round(opwm_cel_api["wind"]["deg"])
        ).capitalize()
    except KeyError:
